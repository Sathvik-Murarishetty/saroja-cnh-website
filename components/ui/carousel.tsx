    ...
    return (
        <div className="[perspective:1200px] [transform-style:preserve-3d]">
            ...
            >
                ...
                {(title || button) && (
                    <article
                        className={`relative p-[2vmin] transition-opacity duration-1000 ease-in-out ${current === index ? "opacity-100 visible" : "opacity-0 invisible"
                            }`}
                    >
                        {title && (
                        ...
                        {button && (
                            <div className="flex justify-center">
                                <button className="mt-4 px-3 py-2 w-fit mx-auto text-xs sm:text-sm text-black bg-white h-10 border border-transparent flex justify-center items-center rounded-xl hover:shadow-lg transition duration-200 shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)]">
                                    {button}
                                </button>
                            </div>
                        )}
...
const CarouselControl = ({ type, title, handleClick }: CarouselControlProps) => {
    return (
        <button
            className={`w-8 h-8 flex items-center mx-2 justify-center bg-neutral-200 dark:bg-neutral-800 border-2 border-transparent rounded-full focus:border-[var(--accent-orange)] focus:outline-none hover:-translate-y-0.5 active:translate-y-0.5 transition duration-200 ${type === "previous" ? "rotate-180" : ""
                }`}
            title={title}
            onClick={handleClick}
        >
